...
client.on('message', function(message) {
	...
	if (mess.startsWith(prefix + 'play')) {
		if (!message.member.voiceChannel) return message.channel.send('**You Are Not In VoiceChannel **');
		// if user is not insert the URL or song title
		if (args.length == 0) {
			let play_info = new Discord.RichEmbed()
				.setAuthor(client.user.username, client.user.avatarURL)
				.setFooter('Request By: ' + message.author.tag)
				.setDescription('**Xplay [SONGNAME OR LINK ]**')
				...
				fetchVideoInfo(id, function(err, videoInfo) {
					...
					let play_info = new Discord.RichEmbed()
						...
						.addField('**Playing**', `**${videoInfo.title}
							  **`)
						.setColor("RANDOM")
                        .addField(`Request by: ${message.author.username}`,`:Cileo1: `)
						.setThumbnail(videoInfo.thumbnailUrl)
